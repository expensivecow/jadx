...
final class CascadingMenuPopup extends MenuPopup implements MenuPresenter, OnKeyListener, OnDismissListener {
    ...
    private final MenuItemHoverListener mMenuItemHoverListener = new MenuItemHoverListener() {
        ...
        public void onItemHoverEnter(@NonNull final MenuBuilder menuBuilder, @NonNull final MenuItem menuItem) {
            ...
            CascadingMenuPopup.this.mSubMenuHoverHandler.removeCallbacksAndMessages(cascadingMenuInfo);
                ...
                CascadingMenuPopup.this.mSubMenuHoverHandler.postAtTime(new Runnable() {
                    public void run() {
                        if (cascadingMenuInfo != null) {
                            ...
                            boolean z = false;
                            cascadingMenuInfo.menu.close(z);
                            CascadingMenuPopup.this.mShouldCloseImmediately = z;
                        }
    ...
    public boolean onKey(View view, int i, KeyEvent keyEvent) {
        boolean action = keyEvent.getAction();
        boolean z = true;
        if (action != z || i != 82) {
            return false;
        }
    ...
    private int getInitialMenuPosition() {
        int i = 1;
        return ViewCompat.getLayoutDirection(this.mAnchorView) == i ? 0 : i;
    }

    private int getNextMenuPosition(int i) {
        int i2 = 1;
        ListView listView = ((CascadingMenuInfo) this.mShowingMenus.get(this.mShowingMenus.size() - i2)).getListView();
        int[] iArr = new int[2];
        listView.getLocationOnScreen(iArr);
        Rect rect = new Rect();
        this.mShownAnchorView.getWindowVisibleDisplayFrame(rect);
        int i3 = 0;
        return this.mLastPosition == i2 ? (iArr[i3] + listView.getWidth()) + i > rect.right ? i3 : i2 : iArr[i3] - i < 0 ? i2 : i3;
    }
    ...
    private void showMenu(@NonNull MenuBuilder menuBuilder) {
        ...
        int measureIndividualMenuWidth = MenuPopup.measureIndividualMenuWidth(menuAdapter, viewGroup, this.mContext, this.mMenuMaxWidth);
        ...
        if (this.mShowingMenus.size() > 0) {
            cascadingMenuInfo = (CascadingMenuInfo) this.mShowingMenus.get(this.mShowingMenus.size() - z);
            findParentViewForSubmenu = findParentViewForSubmenu(cascadingMenuInfo, menuBuilder);
        } else {
            cascadingMenuInfo = viewGroup;
        ...
        if (findParentViewForSubmenu != null) {
            int i;
            int i2;
            createPopupWindow.setTouchModal(z2);
            createPopupWindow.setEnterTransition(viewGroup);
            boolean nextMenuPosition = getNextMenuPosition(measureIndividualMenuWidth);
            boolean z3 = nextMenuPosition == z ? z : z2;
            ...
            int i3 = 5;
            i2 = (this.mDropDownGravity & i3) == i3 ? z3 ? i2 + measureIndividualMenuWidth : i2 - findParentViewForSubmenu.getWidth() : z3 ? i2 + findParentViewForSubmenu.getWidth() : i2 - measureIndividualMenuWidth;
            createPopupWindow.setHorizontalOffset(i2);
            createPopupWindow.setOverlapAnchor(z);
        ...
        if (cascadingMenuInfo == null && this.mShowTitle && menuBuilder.getHeaderTitle() != null) {
            FrameLayout frameLayout = (FrameLayout) from.inflate(R.layout.abc_popup_menu_header_item_layout, listView, z2);
            TextView textView = (TextView) frameLayout.findViewById(16908310);
            frameLayout.setEnabled(z2);
            textView.setText(menuBuilder.getHeaderTitle());
            listView.addHeaderView(frameLayout, viewGroup, z2);
            createPopupWindow.show();
        }
    }
    ...
    private View findParentViewForSubmenu(@NonNull CascadingMenuInfo cascadingMenuInfo, @NonNull MenuBuilder menuBuilder) {
        ...
        return (i2 < 0 || i2 >= listView.getChildCount()) ? view : listView.getChildAt(i2);
    }

    public boolean isShowing() {
        boolean z = false;
        return (this.mShowingMenus.size() <= 0 || !((CascadingMenuInfo) this.mShowingMenus.get(z)).window.isShowing()) ? z : true;
    }
    ...
    public boolean onSubMenuSelected(SubMenuBuilder subMenuBuilder) {
        ...
        do {
            z = true;
            if (it.hasNext()) {
                cascadingMenuInfo = (CascadingMenuInfo) it.next();
    ...
    public void onCloseMenu(MenuBuilder menuBuilder, boolean z) {
        ...
        if (findIndexOfAddedMenu >= 0) {
            ...
            if (i < this.mShowingMenus.size()) {
                ((CascadingMenuInfo) this.mShowingMenus.get(i)).menu.close(z2);
            }
            ...
            if (this.mShouldCloseImmediately) {
                cascadingMenuInfo.window.setExitTransition(viewTreeObserver);
                cascadingMenuInfo.window.setAnimationStyle(z2);
            }
            cascadingMenuInfo.window.dismiss();
            ...
            if (findIndexOfAddedMenu == 0) {
                ...
                ((CascadingMenuInfo) this.mShowingMenus.get(z2)).menu.close(z2);
            }
        }
    }
