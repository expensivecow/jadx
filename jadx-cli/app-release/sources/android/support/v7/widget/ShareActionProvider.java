...
public class ShareActionProvider extends ActionProvider {
    ...
    public void onPrepareSubMenu(SubMenu subMenu) {
        ...
        for (int i2 = i; i2 < min; i2++) {
            ...
            subMenu.add(i, i2, i2, activity.loadLabel(packageManager)).setIcon(activity.loadIcon(packageManager)).setOnMenuItemClickListener(this.mOnMenuItemClickListener);
        }
        if (min < activityCount) {
            subMenu = subMenu.addSubMenu(i, min, min, this.mContext.getString(R.string.abc_activity_chooser_view_see_all));
            for (min = i; min < activityCount; min++) {
                ResolveInfo activity2 = activityChooserModel.getActivity(min);
                subMenu.add(i, min, min, activity2.loadLabel(packageManager)).setIcon(activity2.loadIcon(packageManager)).setOnMenuItemClickListener(this.mOnMenuItemClickListener);
            }
        }
    }
