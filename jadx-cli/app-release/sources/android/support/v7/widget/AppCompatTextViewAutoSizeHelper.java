...
class AppCompatTextViewAutoSizeHelper {
    ...
    private final Context mContext = this.mTextView.getContext();
    ...
    void loadFromAttributes(AttributeSet attributeSet, int i) {
        int resourceId;
        int i2 = 0;
        TypedArray obtainStyledAttributes = this.mContext.obtainStyledAttributes(attributeSet, R.styleable.AppCompatTextView, i, i2);
        if (obtainStyledAttributes.hasValue(R.styleable.AppCompatTextView_autoSizeTextType)) {
            this.mAutoSizeTextType = obtainStyledAttributes.getInt(R.styleable.AppCompatTextView_autoSizeTextType, i2);
        }
        float f = -1.0f;
        float dimension = obtainStyledAttributes.hasValue(R.styleable.AppCompatTextView_autoSizeStepGranularity) ? obtainStyledAttributes.getDimension(R.styleable.AppCompatTextView_autoSizeStepGranularity, f) : f;
        float dimension2 = obtainStyledAttributes.hasValue(R.styleable.AppCompatTextView_autoSizeMinTextSize) ? obtainStyledAttributes.getDimension(R.styleable.AppCompatTextView_autoSizeMinTextSize, f) : f;
        float dimension3 = obtainStyledAttributes.hasValue(R.styleable.AppCompatTextView_autoSizeMaxTextSize) ? obtainStyledAttributes.getDimension(R.styleable.AppCompatTextView_autoSizeMaxTextSize, f) : f;
        if (obtainStyledAttributes.hasValue(R.styleable.AppCompatTextView_autoSizePresetSizes)) {
            resourceId = obtainStyledAttributes.getResourceId(R.styleable.AppCompatTextView_autoSizePresetSizes, i2);
            if (resourceId > 0) {
        ...
        if (!supportsAutoSizeText()) {
            this.mAutoSizeTextType = i2;
        } else if (this.mAutoSizeTextType == 1) {
            if (!this.mHasPresetAutoSizeValues) {
                DisplayMetrics displayMetrics = this.mContext.getResources().getDisplayMetrics();
                resourceId = 2;
                if (dimension2 == f) {
                    dimension2 = TypedValue.applyDimension(resourceId, 12.0f, displayMetrics);
                }
                if (dimension3 == f) {
                    dimension3 = TypedValue.applyDimension(resourceId, 112.0f, displayMetrics);
                }
                if (dimension == f) {
                    dimension = 1.0f;
                }
        ...
        if (supportsAutoSizeText()) {
            switch (i) {
                ...
                case 1:
                    DisplayMetrics displayMetrics = this.mContext.getResources().getDisplayMetrics();
                    int i2 = 2;
                    validateAndSetAutoSizeTextTypeUniformConfiguration(TypedValue.applyDimension(i2, 12.0f, displayMetrics), TypedValue.applyDimension(i2, 112.0f, displayMetrics), 1.0f);
    ...
    void setAutoSizeTextTypeUniformWithPresetSizes(@NonNull int[] iArr, int i) throws IllegalArgumentException {
        if (supportsAutoSizeText()) {
            ...
            this.mHasPresetAutoSizeValues = i2;
            if (setupAutoSizeText()) {
                autoSizeText();
            }
        }
    ...
    private boolean setupAutoSizeUniformPresetSizesConfiguration() {
        ...
        this.mHasPresetAutoSizeValues = length > 0 ? z2 : z;
        if (this.mHasPresetAutoSizeValues) {
            this.mAutoSizeTextType = z2;
            this.mAutoSizeMinTextSizeInPx = (float) this.mAutoSizeTextSizesInPx[z];
            this.mAutoSizeMaxTextSizeInPx = (float) this.mAutoSizeTextSizesInPx[length - z2];
        ...
        if (isAutoSizeEnabled()) {
            if (this.mNeedsAutoSizeText) {
                if (this.mTextView.getMeasuredHeight() > 0 && this.mTextView.getMeasuredWidth() > 0) {
                    int i;
                    boolean z = false;
                    if (((Boolean) invokeAndReturnWithDefault(this.mTextView, "getHorizontallyScrolling", Boolean.valueOf(z))).booleanValue()) {
                        i = 1048576;
                    } else {
                    ...
                    if (i > 0 && height > 0) {
                        synchronized (TEMP_RECTF) {
                            ...
                            if (findLargestTextSizeWhichFits != this.mTextView.getTextSize()) {
                                setTextSizeInternal(z, findLargestTextSizeWhichFits);
                            }
                        }
                    } else {
                        return;
    ...
    private void setRawTextSize(float f) {
        if (f != this.mTextView.getPaint().getTextSize()) {
            ...
            boolean z = false;
            boolean isInLayout = VERSION.SDK_INT >= 18 ? this.mTextView.isInLayout() : z;
            if (this.mTextView.getLayout() != null) {
                this.mNeedsAutoSizeText = z;
                try {
                    Method textViewMethod = getTextViewMethod("nullLayouts");
                    if (textViewMethod != null) {
                        textViewMethod.invoke(this.mTextView, new Object[z]);
                    }
                } catch (Throwable e) {
